...
const ClientDetails = () => {
    ...
    const currentInvoices = clientInvoices.slice(indexOfFirstItem, indexOfLastItem);
    ...
    const totalSpent = clientInvoices
        ...
        .reduce((acc, inv) => acc + (inv.amount || inv.total || 0), 0);
                                    ...
                                    {currentInvoices.length === 0 ? (
                                        ...
                                        currentInvoices.map((inv) => (
                                            <TableRow key={inv.id}>
                                                ...
                                                <TableCell className="text-gray-500">{new Date(inv.created_at || inv.date).toLocaleDateString()}</TableCell>
                                                <TableCell className="font-medium">${Number(inv.total || inv.amount || 0).toLocaleString()}</TableCell>
                                                <TableCell>
                            ...
                            {totalPages > 1 && (
                                <div className="px-6 py-4 flex items-center justify-between border-t border-gray-100 bg-gray-50/50">
                                    <p className="text-xs text-slate-500 font-medium tracking-tight">
                                        Showing <span className="text-slate-900 font-bold">{indexOfFirstItem + 1}</span> to <span className="text-slate-900 font-bold">{Math.min(indexOfLastItem, clientInvoices.length)}</span> of <span className="text-slate-900 font-bold">{clientInvoices.length}</span> invoices
                                    </p>
