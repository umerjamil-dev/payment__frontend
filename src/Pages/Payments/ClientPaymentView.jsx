...
const stripePromise1 = loadStripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY || 'pk_live_51S40UO2zqUuFjAXr7tilzUEgUuBAnm4DT3yXMAnCXU8hdcaDubsJ8h9DfqsKpK8t1SUUu0cFUF0pQac3CjyKQriR00TDZUQfDI');
...
const ClientPaymentView = () => {
  ...
  const [activeStripeGateway, setActiveStripeGateway] = useState('stripe1'); // 'stripe1' or 'stripe2'
  ...
  const handlePaymentSuccess = (methodName) => {
    ...
    // Update global state and persist to backend
  ...
  return (
    <div className="inv-root">
            ...
            {isPaid ? (
              ...
              <>
                <div className="inv-gateways">
                  {(parsedGateways?.stripe1 || (!parsedGateways && gateways?.stripe1?.enabled !== false)) && (
                    <button className="inv-gateway-btn inv-gateway-stripe" onClick={() => { setActiveStripeGateway('stripe1'); setIsStripeOpen(true); }}>
                  ...
                  {(parsedGateways?.stripe2 || (!parsedGateways && gateways?.stripe2?.enabled !== false)) && (
                    <button className="inv-gateway-btn inv-gateway-stripe" style={{ background: 'linear-gradient(135deg, #4F46E5, #3730A3)' }} onClick={() => { setActiveStripeGateway('stripe2'); setIsStripeOpen(true); }}>
                      <div className="inv-gateway-left">
                        <div className="inv-gateway-icon"><CreditCard size={18} color="#fff" /></div>
                        <div style={{ textAlign: 'left' }}>
                          <div className="inv-gateway-method">Pay with Card</div>
      ...
      {isStripeOpen && (
        <Elements stripe={activeStripeGateway === 'stripe1' ? stripePromise1 : stripePromise2} key={activeStripeGateway}>
          <StripeModal
            isOpen={isStripeOpen}
