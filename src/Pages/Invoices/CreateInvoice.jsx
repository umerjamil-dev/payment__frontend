        ...
        if (newInvoice) {
            ...
            try {
                ...
                const templateParams = {
                    ...
                    email: client.email, // Restored for EmailJS dashboard compatibility
                    to_name: client.name,
                    brand_name: brandName,
                    brand_email: brandEmail,
                    invoice_id: displayId,
                    total_amount: `$${Number(total || 0).toFixed(2)}`,
                    payment_link: paymentLink,
                    reply_to: brandEmail,
                    company_name: brandName,
                    message_header: `Welcome to ${brandName}, ${client.name}!`,
                    message_body: `We hope you are doing well. Your invoice #${displayId} for a total of $${Number(total || 0).toFixed(2)} has been generated and is ready for payment.`,

                    best_regards: `Best regards,\nThe ${brandName} Team`
                };


                console.log('Dispatching via EmailJS:', { SERVICE_ID, TEMPLATE_ID, templateParams });
                const response = await emailjs.send(SERVICE_ID, TEMPLATE_ID, templateParams);

                if (response.status === 200) {
                    toast.success('Email sent successfully to ' + client.email);
                } else {
                    throw new Error(`EmailJS Error (${response.status}): ${response.text || 'Unknown error'}`);
                }
            } catch (error) {
                console.error('Email Dispatch Error Details:', error);
                
                const brand = brands.find(b => String(b.id) === String(selectedBrandId)) || { name: 'CRM' };
                const brandName = brand.name || 'Our CRM';
                const displayId = (1010 + Number(newInvoice.id || 0));
                const errorMsg = error?.text || error?.message || (typeof error === 'string' ? error : 'Network error or Invalid API Keys');
                
                toast.error('Email Sync Failed: ' + errorMsg);
                toast.error('Opening local email app as backup...');

                // Fallback to mailto link - PROPER ENGLISH
                const subject = encodeURIComponent(`Invoice #${displayId} from ${brandName}`);
                const body = encodeURIComponent(`Hello ${client.name},\n\nWelcome to ${brandName}!\n\nWe hope you are doing well. Your invoice #${displayId} for $${Number(total || 0).toFixed(2)} has been generated.\n\nPlease use the link below to view and complete the payment:\n\n${paymentLink}\n\nBest regards,\nThe ${brandName} Team`);
