...
export const CRMProvider = ({ children }) => {
    ...
    const fetchData = async (isInitial = false) => {
        ...
        try {
            ...
            // toast.error('Failed to sync with Backend'); // Avoid spamming during polling
        } finally {
    ...
    useEffect(() => {
        ...
        // Custom API Polling (since Realtime isn't available on the custom backend)
        const pollInterval = setInterval(() => {
            fetchData(false);
        }, 10000); // 10 seconds polling for pseudo-realtime
    ...
    const updateInvoiceStatus = async (invoiceId, status, paymentMethod = null) => {
        // First find the real numeric ID from our current state (which handles UUID/ID mapping)
        const invoice = invoices.find(inv => 
            String(inv.id) === String(invoiceId) || 
            String(inv.uuid) === String(invoiceId)
        );

        if (!invoice) {
            console.warn('Invoice for status update not found in state:', invoiceId);
            return;
        }

        const realNumericId = invoice.id;

        setInvoices(prev => prev.map(inv =>
            inv.id === realNumericId ? {
                ...inv,
                status,
        ...
        // Persist to Backend using numeric ID
        const updateData = {
            status,
        ...
        const { error } = await insforge.database.from('invoices').update(updateData).match({ id: realNumericId });
        if (error) {
            ...
            if (status === 'Paid') {
                ...
                const displayId = 1010 + Number(realNumericId);
                
                if (client) {
                    const newActivity = {
                        type: 'payment',
                        title: `Payment Received - ${displayId}`,
                        description: `Client ${client.name} paid ${invoice.total || invoice.amount} via ${paymentMethod || 'Manual'}`,
                        status: 'success'
                    };
                    addActivity(newActivity);
                }
                toast.success(`Invoice ${displayId} marked as Paid!`);
            } else {
    ...
    const getDashboardStats = () => {
        ...
        // Brand Performance (Revenue per Brand)
        const brandPerformance = brands.map(brand => {
            const brandInvoices = invoices.filter(inv => String(inv.brand_id || inv.brandId) === String(brand.id) && inv.status === 'Paid');
            return {
                name: brand.name,
                value: brandInvoices.reduce((acc, inv) => acc + Number(inv.total || 10), 0),
                color: brand.color || '#5B4FE9'
            };
        }).filter(b => b.value > 0).sort((a, b) => b.value - a.value);

        // Top Clients (Revenue per Client)
        const topClients = clients.map(client => {
            const clientInvoices = invoices.filter(inv => String(inv.client_id || inv.clientId) === String(client.id) && inv.status === 'Paid');
            return {
                name: client.name,
                value: clientInvoices.reduce((acc, inv) => acc + Number(inv.total || 0), 0)
            };
